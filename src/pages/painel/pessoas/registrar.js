...
function RegistrarPessoa({ token }) {
    ...
    const handleUserRegisterSubmit = async (event) => {
        ...
        if (response.status === 200) {
            ...
            showModal(
                <MessageModal
                    icon="success" title="Usuário Registrado" message="O usuário foi registrado com successo."
                    buttons={[ { label: "Fechar", action: closeModal } ]}
                ></MessageModal>
            );

            router.push(`/painel/pessoas/${response.userId}`);

        } else {
            
            if (response.errors && Object.keys(response.errors).length > 0) {
                setUserRegisterErrors(response.errors);
            } else {
                showModal(
                    <MessageModal 
        ...
        <>
            ...
            <div className="w-full flex flex-col justify-center items-start gap-6 border-b border-neutral-800 scale-right-to-left">
                ...
                <div className="w-full flex flex-col justify-start items-start pr-4 pb-3 gap-1">
                    <h1 className="font-lgc text-3xl sm:text-4xl slide-up-fade-in opacity-0" style={{ animationDelay: "400ms" }}>Registrar Pessoa</h1>
            ...
            <div className="flex flex-col justify-start items-start gap-6 py-6">
                ...
                <div className="w-full max-w-2xl flex flex-row items-center gap-2 smooth-slide-down-fade-in opacity-0" style={{ animationDelay: "300ms" }}>
                    <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
                ...
                <form onSubmit={handleUserRegisterSubmit} className="w-full max-w-2xl flex flex-col gap-6 px-4 py-5 bg-neutral-100 rounded-md smooth-slide-down-fade-in opacity-0" style={{ animationDelay: "400ms" }}>
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col xl:flex-row items-start gap-6">
                            ...
                            <AdvancedSelect name="role" label="Cargo" onChange={handleUserRegisterInputChange} error={userRegisterErrors?.role} bgColor="bg-neutral-100"
                                options={[
                    ...
                    <div className="w-full flex flex-row justify-end items-center mt-2">
                        <button disabled={userRegisterLoading} className="w-full xl:w-56 flex flex-row justify-center items-center gap-3 font-lgc font-bold text-lg p-2 rounded-md text-white bg-red-500 hover:bg-red-600 disabled:bg-red-600 disabled:cursor-default transition-all" type="submit">
                            { userRegisterLoading ? (
