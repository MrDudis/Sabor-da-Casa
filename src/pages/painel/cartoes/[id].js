...

                    {
                        card?.userId ? (
                            <div className="w-full flex flex-row items-center px-4 py-3 gap-4 bg-neutral-100 rounded-md smooth-slide-down-fade-in opacity-0" style={{ animationDelay: "400ms" }}>
                                ...
                                <p className="w-full font-lgc text-lg">Vinculado com <span className="font-bold">{card.user?.name}</span></p>

                                <Link href={`/painel/pessoas/${card.user?.id}`} className="bg-neutral-100 hover:bg-neutral-200 cursor-pointer p-2 rounded-md transition-all">
                                    <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
