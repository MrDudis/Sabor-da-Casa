...
import ModalContext from "../../../providers/modal/ModalContext.js";

import { MessageModal } from "../../elements/modal/Modal.js";
...
export default function Dropdown({ showDropdown, setShowDropdown }) {
    ...
    const handleLogoutConfirm = () => {
        ...
        showModal(
            <MessageModal
                icon="warning" title="Sair da Conta?" message="Tem certeza que deseja sair da sua conta? Será necessário fazer login novamente para acessar o painel."
                buttons={[ { label: "Sair", action: handleLogout }, { label: "Cancelar", action: closeModal } ]}
            ></MessageModal>
        );
    ...
    return (
        ...
        <div className={`${dropdownDisplay} absolute z-30 w-full top-12 p-1.5 rounded-md flex-col justify-center items-end bg-stone-800 shadow-2xl ${showDropdown ? "dropdown-slide-in" : "dropdown-slide-out"}`}>
            ...
            <div onClick={handleLogoutConfirm} className="w-full flex flex-row items-center gap-2 p-1.5 rounded-md hover:bg-stone-700 transition-all">
